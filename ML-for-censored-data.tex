...
\begin{abstract}
Models for predicting the risk of various health events over a certain time frame (e.g., having a heart attack in the next 5 years) based on individual patient characteristics are important tools for managing patient care. Because electronic health data from large health care systems provide access to data on large, heterogeneous, and contemporaneous patient populations, EHD are advantageous and increasingly available sources to train risk prediction models. Machine learning approaches to estimate the risk are attractive because many techniques automatically detect non-linear relationships between risk factors and health outcomes and differing effects from different patient subgroups. However, in such data, the event status may be unknown for some individuals, as the event time is right-censored
due to disenrollment and incomplete follow-up. There have been many {\em{ad hoc}} approaches suggested to handle right-censored outcomes across a variety of machine learning outcomes. We illustrate a unified approach to account for right-censored outcomes using by weighting observations by the inverse probability probability of censoring (IPC). We show that our approach can lead to better predictive performance than machine learning techniques using {\em{ad hoc}} approaches to right-censoring. Our techniques are motivated by and illustrated on predicting the risk of having a cardiovascular event with 5 years of enrolling in a large U.S. Midwestern health care system.
\end{abstract}
