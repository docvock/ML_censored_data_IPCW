...
Table \ref{table:calib-cindex} shows how different approaches to handling censored observations affect the predicted event rate, calibration statistic, and C-index of the machine learning techniques described in Section \ref{sect:examples}. Results from the Cox proportional hazards model \citep{Cox_1972}, which has been used as the basis for most risk prediction model are included for comparison. It is clear from Table~\ref{table:calib-cindex} that the \emph{-Discard}, \emph{-Zero}, and \emph{-Split} variants of each technique are poorly calibrated; as expected, the \emph{-Discard} approaches  yielding biased estimates of both the overall and subgroup-specific risks. The amount of improvement in reclassification performance due to using IPCW is technique-dependent; C-index is better for \emph{Tree-IPCW} than \emph{Tree-Zero} and \emph{Tree-Discard}, but there is no apparent effect for logistic regression.
