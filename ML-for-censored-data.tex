...
The total number of first CV events recorded within the follow-up period is XXXX; the Kaplan-Meier 5-year event rate for the entire analysis cohort is XXX\%.
...
The full training dataset consists of XXXX patients (75\% of the entire analysis population) drawn at random from the cohort. XX\% were censored, so that \emph{Discard-} models were trained on XXXX subjects. The performance of all models is evaluated based on the risk predictions of the remaining XXX patients not included in any training set. The calibration and discrimination of the models evaluated on the test set are summarized in Table~\ref{table:resSummary}. 
